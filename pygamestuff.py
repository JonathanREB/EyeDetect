...
##        print self.crossrect.left, "is the left"
    ...
    def record(self, inputTuple):
        if self.crossrect.centerx < GetSystemMetrics(0)/2:
            side = 0
        else:
            side = 1
        self.result.append(list(inputTuple)+[self.crossrect.centerx,self.crossrect.centery]+[side])


    def write(self):
        fo = open("training_model.csv", "w")
